...
def main() -> int:
    ...
    ap.add_argument(
        "--key",
        default=None,
        help="Path to the ledger integrity key. Defaults to runtime_keys/ledger_hmac.key.",
    )
    ap.add_argument(
        "--legacy",
        action="store_true",
        help="Verify UNSIGNED legacy manifests (chain/consistency only). NOT tamper-resistant.",
    )
    args = ap.parse_args()

    # Gamma Hardening v1: require a verified per-record signature unless --legacy.
    key = None
    if not args.legacy:
        from gamma_ledger_integrity import load_key, verify_record, DEFAULT_KEY_PATH
        try:
            key = load_key(args.key) if args.key else load_key(DEFAULT_KEY_PATH)
        except FileNotFoundError:
            print("[fatal] no ledger integrity key found; pass --key or --legacy")
            return 2
    ...
    with open(args.manifest, "rb") as fh:
        for raw in fh:
            ...
            # 0. record authenticity (Gamma Hardening v1): the keyed MAC covers
            #    every field, so forged or in-place-edited records are rejected.
            if key is not None and not verify_record(rec, key):
                signature_fail += 1
                if signature_fail <= 5:
                    print(f"  [signature] seq={rec.get('seq')} "
                          f"missing/invalid record signature")
    ...
    print(f"signature failures  : {signature_fail if key is not None else 'SKIPPED (--legacy)'}")
